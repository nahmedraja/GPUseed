...
void gasal_set_device(int gpu_select)
{
	...
	*/
	...
	CHECKCUDAERROR(cudaGetDeviceCount(&num_devices), "cudaGetDeviceCount");
	...
	if (num_devices > 0) {
		...
		CHECKCUDAERROR(cudaGetDeviceProperties(&properties, gpu_select));
		fprintf(stderr, "Selected device %d : %s\n", gpu_select, properties.name);
		CHECKCUDAERROR(cudaSetDevice(gpu_select), "cudaSetDevice");
	}
}
