...
__constant__ bwtint_t L2_gpu[5];
...
//__device__ inline bwtint_t bwt_occ_gpu(const bwt_t bwt, bwtint_t k, uint8_t c)
//{
//	bwtint_t occ;
//
//	if (k == bwt.seq_len) return L2_gpu[c+1] - L2_gpu[c];
//	if (k == (bwtint_t)(-1)) return 0;
//	if (k >= bwt.primary) --k; // because $ is not in bwt/__device__ inline uint2 find_occ_gpu(const bwt_t bwt, bwtint_t l, bwtint_t u, uint8_t c)
//{
//	bwtint_t occ_l = 0, occ_u = 0; //occ_l_1 = 0, occ_l_2 = 0, occ_l_3 = 0, occ_u_1 = 0;
//
...
//	if (l == (bwtint_t)(-1)) return make_uint2 (0, bwt_occ_gpu(bwt, u, c));
//	if (u == (bwtint_t)(-1)) return make_uint2 (bwt_occ_gpu(bwt , l, c), 0);
//	if (l >= bwt.primary) --l;
//	if (u >= bwt.primary) --u;
//
//	bwtint_t kl = l / OCC_INTERVAL;
//	bwtint_t ku = u /OCC_INTERVAL;
//
...
//
//	n = bwt_occ_intv(bwt, k)[c];
//	bwtint_t k_pos = k&(OCC_INTERVAL-1);
...
//__device__ inline uint2 find_occ_gpu(const bwt_t bwt, bwtint_t l, bwtint_t u, uint8_t c)
//{
//	bwtint_t occ_l = 0, occ_u = 0; //occ_l_1 = 0, occ_l_2 = 0, occ_l_3 = 0, occ_u_1 = 0;
//
//	//if ((l+1) > u || c > 3)  return make_uint2(l+1, u);
...
//	bwtint_t ku = u /OCC_INTERVAL;
//	occ_l = bwt_occ_intv(bwt, l)[c];
//	bwtint_t l_pos = l&(OCC_INTERVAL-1);
//	bwtint_t u_pos = u&(OCC_INTERVAL-1);
//	bwtint_t l_pos_reg = (l_pos>>4);
//	bwtint_t u_pos_reg = (u_pos>>4);
...
//__device__ inline bwtint_t bwt_inv_psi_gpu(const bwt_t bwt, bwtint_t k) {
...
__device__ inline bwtint_t bwt_inv_psi_gpu(const bwt_t bwt, bwtint_t k) {
	...
	bwtint_t n = bwt_occ_intv(bwt, k)[c];
...
//__device__ int seed_intervals_gpu(uint8_t *read, int read_len, bwtint_t *seed_interval_l_gpu, bwtint_t *seed_interval_u_gpu,
//		int *seed_read_begin_gpu, int *seed_read_end_gpu, int min_seed_size, bwt_t bwt) {
//
//	uint8_t ch;
...
//		int i;
//		bwtint_t l, u;
//		bwtint_t prev_l = 0, prev_u = bwt.seq_len;
...
//}
//
//__global__ void calc_smems(uint8_t *read_batch_gpu, int *read_sizes_gpu, int *read_offsets_gpu, bwtint_t *seed_interval_l_gpu, bwtint_t *seed_interval_u_gpu,
//		int *seed_read_begin_gpu, int *seed_read_end_gpu, int *num_seeds, int min_seed_size, bwt_t bwt, int actual_read_batch_size, int (MAX_READ_LENGTH - min_seed_size)) {
//
...
__global__ void locate_seeds_gpu(uint32_t *seed_ref_pos_fow_rev_gpu, bwt_t bwt, uint32_t n_seeds_sum_fow_rev) {
...
__global__ void locate_seeds_gpu_wrapper(int2 *final_seed_read_pos_fow_rev, uint32_t *seed_sa_idx_fow_rev_gpu, uint32_t *n_seeds_fow_rev_scan, uint2 *seed_intervals_fow_rev, int2 *seed_read_pos_fow_rev,uint32_t *n_smems_sum_fow_rev, uint32_t *n_seeds_sum_fow_rev, bwt_t bwt) {
...
__global__ void locate_seeds_gpu_wrapper_mem(int2 *final_seed_read_pos_fow_rev, uint32_t *seed_sa_idx_fow_rev_gpu, uint32_t *n_seeds_fow_rev_scan, uint2 *seed_intervals_fow_rev, int2 *seed_read_pos_fow_rev,uint32_t *n_smems_sum_fow_rev, uint32_t *n_seeds_sum_fow_rev, uint32_t *n_ref_pos_fow_rev, bwt_t bwt) {
...
//__global__ void locate_seeds_gpu(uint2 *seed_read_pos_gpu, uint2 *seed_read_pos_rev_gpu, uint2 *final_seed_read_pos_gpu, uint2 *final_seed_read_pos_rev_gpu, uint32_t *seed_ref_pos_fow_gpu, uint32_t *seed_ref_pos_rev_gpu, uint32_t *n_seeds_fow_scan, uint32_t *n_seeds_rev_scan, uint32_t *smem_intv_l_fow, uint32_t *smem_intv_l_rev, bwt_t bwt, uint32_t n_smems_sum_fow, uint32_t n_smems_sum_rev, uint32_t n_ref_pos) {
//
//        int tid = (blockIdx.x*blockDim.x) + threadIdx.x;
...
//        	for (j = 0 ; j < n_locs; j++) {
//        		int k, itr = 0;
//        		bwtint_t u = seed_interval_u_gpu[intv_idx + i] + j;
//        		//for(k = 0; k < 100; k++) {
//        		while(u % bwt.sa_intv){
//        			itr++;
...
//__global__ void finalize_seeds_gpu(int *read_sizes_gpu, bwtint_t *seed_interval_l_gpu, bwtint_t *seed_interval_u_gpu,
...
//__global__ void filter_seed_intervals_gpu(int *read_sizes_gpu, bwtint_t *seed_interval_l_gpu, bwtint_t *seed_interval_u_gpu,
...
//		int2 *seed_read_pos_fow, int2 *seed_read_pos_rev, uint32_t *read_num, uint32_t *read_idx, /*uint2* pre_calc_intervals,*/ int min_seed_size, bwt_t bwt, /*int pre_calc_seed_len,*/ int n_tasks) {
//
//	int tid = (blockIdx.x*blockDim.x) + threadIdx.x;
//	if (tid >= 2*n_tasks) return;
//	int thread_read_num = read_num[tid%n_tasks];
...
//		int2 *seed_read_pos_fow_rev, uint32_t *read_num, uint32_t *read_idx, uint32_t *is_smem_fow_rev_flag, uint2* pre_calc_intervals, uint32_t *n_smems_fow,  uint32_t *n_smems_rev,int min_seed_size, bwt_t bwt, int pre_calc_seed_len, int n_tasks) {
//
//	int tid = (blockIdx.x*blockDim.x) + threadIdx.x;
//	if (tid >= 2*n_tasks) return;
//	int thread_read_num = read_num[tid%n_tasks];
...
//		int2 *seed_read_pos_fow_rev, uint32_t *read_num, uint32_t *read_idx, uint32_t *is_smem_fow_rev_flag, uint2* pre_calc_intervals, uint32_t *n_smems_fow,  uint32_t *n_smems_rev,int min_seed_size, bwt_t bwt, int pre_calc_seed_len, int n_tasks) {eg_pos << 2)))&15;
////			/*unknown bases*/
////			if (base > 3) {
////				break;
////			}
...
__global__ void pre_calc_seed_intervals_gpu(uint2* pre_calc_intervals, int pre_calc_seed_len, bwt_t bwt, int n_tasks) {
...
void bwt_restore_bwt(const char *fn, bwt_t *bwt)
{
	FILE *fp;
	fp = fopen(fn, "rb");
	if (fp == NULL){
...
int main(int argc, char *argv[]) {
	...
	strcpy(str, prefix); strcat(str, ".bwt");  bwt_restore_bwt(str, &bwt);
	free(str);
...
//	}
    ...
    cudaMalloc(&(bwt_gpu.bwt), bwt.bwt_size*sizeof(bwtint_t));
    cudaMemcpy(bwt_gpu.bwt, bwt.bwt, bwt.bwt_size*sizeof(bwtint_t),cudaMemcpyHostToDevice);
    cudaMalloc(&(bwt_gpu.sa), bwt.n_sa*sizeof(bwtint_t));
    cudaMemcpy(bwt_gpu.sa, bwt.sa, bwt.n_sa*sizeof(bwtint_t),cudaMemcpyHostToDevice);
    ...
    cudaMemcpyToSymbol(L2_gpu, bwt.L2, 5*sizeof(bwtint_t), 0, cudaMemcpyHostToDevice);
    ...
    fprintf(stderr, "\n-----------------------------------------------------------------------------------------------------------\n");
	FILE *read_file = fopen(argv[optind + 1], "r");
...
//			}
		...
		total_batch_load_time += (realtime() - loading_time);
		fprintf(stderr,"A batch of %d reads loaded from file in %.3f seconds\n", total_reads, realtime() - loading_time);
...
//		cudaMalloc(&is_smem_fow_rev_flag, 2*(read_batch_size_8 - ((min_seed_size-1)*total_reads))*sizeof(uint32_t));
		...
		double gpu_batch_time = realtime();
		double mem_time0 = realtime();
		...
		mem_time0 = realtime() - mem_time0;
		double batch_prep_time = realtime();
		int BLOCKDIM = 128;
		double rev_pack_time_start = realtime();
		...
		cudaDeviceSynchronize();
		double rev_pack_time = realtime() - rev_pack_time_start;
		double assign_threads_for_fow_pack_time_start = realtime();
...
//		}
//		fprintf(stderr, "\n");
		double fow_pack_time_start = realtime();
		...
		cudaDeviceSynchronize();
		double fow_pack_time = realtime() - fow_pack_time_start;
		double assign_threads_time_start = realtime();
		...
		cudaDeviceSynchronize();
		double assign_threads_time = realtime() - assign_threads_time_start;
...
//		}
		total_batch_prep_time += (realtime() - batch_prep_time);

		fprintf(stderr,"Batch prepared for processing on GPU in %.3f seconds\n", realtime() - batch_prep_time);
...
//		cudaMalloc(&seed_intervals_u_gpu, (MAX_READ_LENGTH - min_seed_size)*(2*actual_read_batch_size)*sizeof(bwtint_t));
...
//		bwtint_t *seed_intervals_l = (bwtint_t*)calloc((MAX_READ_LENGTH - min_seed_size)*(2*actual_read_batch_size), sizeof(bwtint_t));
...
//		int *seed_ref_begin = (int*)malloc(MAX_SEEDS_PER_READ*count_seed_intervals_gpu(2*actual_read_batch_size)*sizeof(bwtint_t));
...
//
//		bwtint_t *seed_intervals_l_gpu,  *seed_intervals_u_gpu, *seed_ref_begin_gpu;
//		int *seed_read_begin_gpu, *seed_read_end_gpu, *num_seeds_gpu;
//
//		cudaMalloc(&seed_intervals_l_gpu, (MAX_READ_LENGTH - min_seed_size)*(2*actual_read_batch_size)*sizeof(bwtint_t));
//		cudaMalloc(&seed_intervals_u_gpu, (MAX_READ_LENGTH - min_seed_size)*(2*actual_read_batch_size)*sizeof(bwtint_t));
//		cudaMalloc(&seed_read_begin_gpu, (MAX_READ_LENGTH - min_seed_size)*(2*actual_read_batch_size)*sizeof(int));
//		cudaMalloc(&seed_read_end_gpu, (MAX_READ_LENGTH - min_seed_size)*(2*actual_read_batch_size)*sizeof(int));
//		cudaMalloc(&num_seeds_gpu, (2*actual_read_batch_size)*sizeof(int));
//		cudaMalloc(&seed_ref_begin_gpu, MAX_SEEDS_PER_READ*(2*actual_read_batch_size)*sizeof(bwtint_t));
		...
		cudaDeviceSynchronize();
		fprintf(stderr,"\tIntervals of SMEM seeds computed in %.3f seconds on GPU\n", realtime() - find_seeds_time);
		total_find_seed_intervals_time += realtime() - find_seeds_time;

//		cub::DeviceReduce::Sum(cub_scan_temp_storage, cub_scan_storage_bytes, n_smems_fow, &n_smems_sum_gpu[0], total_reads);
//		cudaDeviceSynchronize();
//		//fprintf(stderr,"\tn_smems_fow summed in %.3f seconds\n", realtime() - n_smems_fow_sum_time);
//
//		double n_smems_rev_sum_time = realtime();
//		cub::DeviceReduce::Sum(cub_scan_temp_storage, cub_scan_storage_bytes, n_smems_rev, &n_smems_sum_gpu[1], total_reads);
//		cudaDeviceSynchronize();
//		fprintf(stderr,"\tn_smems_rev summed in %.3f seconds\n", realtime() - n_smems_rev_sum_time);
//
//		uint32_t n_smems_sum[2];
////	cudaMemcpy(n_smems_sum, n_smems_sum_gpu, 2*sizeof(uint32_t), cudaMemcpyDeviceToHost);
//
//
//		fprintf(stderr, "n_smems_sum_fow = %d, n_smems_sum_rev = %d\n", n_smems_sum[0], n_smems_sum[1]);

		double n_smems_fow_max_time = realtime();
		cub::DeviceReduce::Max(cub_scan_temp_storage, cub_scan_storage_bytes, n_smems_fow, &n_smems_max_gpu[0], total_reads);
		cudaDeviceSynchronize();
		fprintf(stderr,"\tMax in n_smems_fow found in %.3f seconds\n", realtime() - n_smems_fow_max_time);
		double n_smems_rev_max_time = realtime();
		cub::DeviceReduce::Max(cub_scan_temp_storage, cub_scan_storage_bytes, n_smems_rev, &n_smems_max_gpu[1], total_reads);
		cudaDeviceSynchronize();
		fprintf(stderr,"\tMax in n_smems_rev found in %.3f seconds\n", realtime() - n_smems_rev_max_time);
...
//		if (err != cudaSuccess) {
//			fprintf(stderr, "flagged failed\n");
//		}

//		double n_smems_fow_sum_time = realtime();
//		cub::DeviceReduce::Sum(cub_scan_temp_storage, cub_scan_storage_bytes, n_smems_fow, &n_smems_sum_gpu[0], total_reads);
//		cudaDeviceSynchronize();
//		fprintf(stderr,"\tn_smems_fow summed in %.3f seconds\n", realtime() - n_smems_fow_sum_time);
//
...
//		fprintf(stderr,"\tn_smems_rev summed in %.3f seconds\n", realtime() - n_smems_rev_sum_time);
		...
		double n_smems_fow_rev_scan_time = realtime();
...
//			seed_intervals_fow_rev_gpu = swap2;
		...
		//double read_pos_sort_time = realtime();
...
//		cudaDeviceSynchronize();
//		fprintf(stderr,"\tread_pos sorted in %.3f seconds\n", realtime() - read_pos_sort_time);

		fprintf(stderr,"\tSMEM seeds filtered in %.3f seconds on GPU\n", realtime() - n_smems_fow_max_time);
		total_filter_seed_intervals_time += realtime() - n_smems_fow_max_time;


//		uint32_t n_smems_max[2];
//		cudaMemcpy(n_smems_max, n_smems_max_gpu, 2*sizeof(uint32_t), cudaMemcpyDeviceToHost);
//
//		fprintf(stderr, "n_smems_max_fow = %d, n_smems_max_rev = %d\n", n_smems_max[0], n_smems_max[1]);
		//N_BLOCKS = (2*((n_smems_max[0] > n_smems_max[1] ? n_smems_max[0] : n_smems_max[1])*total_reads) + BLOCKDIM - 1)/BLOCKDIM;
//		double filter_seeds_fow_time = realtime();
//		N_BLOCKS = ((n_smems_max[0]*n_smems_max[0]*total_reads) + BLOCKDIM - 1)/BLOCKDIM;
//		filter_seeds_gpu<<<N_BLOCKS, BLOCKDIM>>>(seed_intervals_fow_gpu, seed_read_pos_fow_gpu, n_smems_fow,  n_smems_max[0],  1, min_seed_size, n_smems_max[0]*n_smems_max[0]*total_reads);
//		cudaDeviceSynchronize();
//		fprintf(stderr,"\tSMEM seeds on forward read filtered in %.3f seconds on GPU\n", realtime() - filter_seeds_fow_time);
//
//		double filter_seeds_rev_time = realtime();
//		N_BLOCKS = ((n_smems_max[1]*n_smems_max[1]*total_reads) + BLOCKDIM - 1)/BLOCKDIM;
//		filter_seeds_gpu<<<N_BLOCKS, BLOCKDIM>>>(seed_intervals_rev_gpu, seed_read_pos_rev_gpu, n_smems_rev, n_smems_max[1],  0, min_seed_size, n_smems_max[1]*n_smems_max[1]*total_reads);
//		cudaDeviceSynchronize();
//		fprintf(stderr,"\tSMEM seeds on reverse read filtered in %.3f seconds on GPU\n", realtime() - filter_seeds_rev_time);
//
//		double finalize_seeds_time = realtime();
//		N_BLOCKS = ((n_smems_max[0]*n_smems_max[1]*total_reads) + BLOCKDIM - 1)/BLOCKDIM;
//		finalize_seeds_gpu<<<N_BLOCKS, BLOCKDIM>>>(seed_intervals_fow_gpu, seed_intervals_rev_gpu, seed_read_pos_fow_gpu, seed_read_pos_rev_gpu, n_smems_fow, n_smems_rev, n_smems_max[0],  n_smems_max[1], min_seed_size, n_smems_max[0]*n_smems_max[1]*total_reads);
//		cudaDeviceSynchronize();
//		fprintf(stderr,"\tCommon seeds between forward and reverse read filtered in %.3f seconds on GPU\n", realtime() - finalize_seeds_time);

		//total_filter_seed_intervals_time += realtime() - n_smems_fow_max_time;


		double locate_seeds_time = realtime();
...
//		n_smems_sum[1]=n_smems_sum[1]/2;
		...
		//cudaDeviceSynchronize();
		//fprintf(stderr,"\tNumber of seeds counted in %.3f seconds on GPU\n", realtime() - count_seeds_time);





		double n_seeds_fow_rev_sum_time = realtime();
...
//		fprintf(stderr,"\tn_seeds_rev summed in %.3f seconds\n", realtime() - n_seeds_rev_sum_time);
		...
		cudaDeviceSynchronize();
		fprintf(stderr,"\tn_seeds_fow_rev scanned in %.3f seconds on GPU\n", realtime() - n_seeds_fow_rev_scan_time);



//		cudaMalloc(&seed_sa_idx_fow_gpu, n_seeds_sum[0]*sizeof(uint32_t));n_seeds_fow
//		cudaMalloc(&seed_sa_idx_rev_gpu, n_seeds_sum[1]*sizeof(uint32_t));
//		cudaMalloc(&seed_read_pos_fow_gpu, n_seeds_sum[0]*sizeof(int2));
//		cudaMalloc(&seed_read_pos_rev_gpu, n_seeds_sum[1]*sizeof(int2));

		//N_BLOCKS = ((n_smems_sum[0] + n_smems_sum[1])*THREADS_PER_SMEM  + BLOCKDIM - 1)/BLOCKDIM;
//
//		double final_seeds_time = realtime();
//		finalize_seeds_gpu<<<num_blocks, threads_per_block>>>(read_sizes_gpu, seed_intervals_l_gpu, seed_intervals_u_gpu,
//				seed_read_begin_gpu, seed_read_end_gpu, num_seeds_gpu, (2*actual_read_batch_size), min_seed_size);
//		cudaDeviceSynchronize();
//		fprintf(stderr,"\tSMEM seeds finalized in %.3f seconds on GPU\n", realtime() - final_seeds_time);
//
		...
		cudaDeviceSynchronize();
		fprintf(stderr,"\tSeeds located on ref in in %.3f seconds\n", realtime() - locate_seeds_time);
		total_locate_seeds_time += realtime() - locate_seeds_time;
...
//		cudaMemcpy(seed_intervals_fow, seed_intervals_fow_gpu, (MAX_READ_LENGTH - min_seed_size)*(total_reads)*sizeof(uint2), cudaMemcpyDeviceToHost);
//		cudaMemcpy(seed_intervals_rev, seed_intervals_rev_gpu, (MAX_READ_LENGTH - min_seed_size)*(total_reads)*sizeof(uint2), cudaMemcpyDeviceToHost);
//		cudaMemcpy(seed_read_pos_fow, seed_read_pos_fow_gpu, (MAX_READ_LENGTH - min_seed_size)*(total_reads)*sizeof(uint2), cudaMemcpyDeviceToHost);
//		cudaMemcpy(seed_read_pos_rev, seed_read_pos_rev_gpu, (MAX_READ_LENGTH - min_seed_size)*(total_reads)*sizeof(uint2), cudaMemcpyDeviceToHost);

//		cudaMemcpy(seed_intervals_u, seed_intervals_u_gpu, (MAX_READ_LENGTH - min_seed_size)*(2*actual_read_batch_size)*sizeof(bwtint_t), cudaMemcpyDeviceToHost);
//		cudaMemcpy(seed_read_begin, seed_read_begin_gpu, (MAX_READ_LENGTH - min_seed_size)*(2*actual_read_batch_size)*sizeof(int), cudaMemcpyDeviceToHost);
//		cudaMemcpy(seed_read_end, seed_read_end_gpu, (MAX_READ_LENGTH - min_seed_size)*(2*actual_read_batch_size)*sizeof(int), cudaMemcpyDeviceToHost);
//		cudaMemcpy(num_seeds, num_seeds_gpu, (2*actual_read_batch_size)*sizeof(int), cudaMemcpyDeviceToHost);
//		cudaMemcpy(seed_ref_begin, seed_ref_begin_gpu, MAX_SEEDS_PER_READ*(2*actual_read_batch_size)*sizeof(bwtint_t), cudaMemcpyDeviceToHost);



		mem_time1 = (realtime() - mem_time1);
		fprintf(stderr,"\tTime spent in cudaMemcpy is %.3f seconds\n", mem_time0 + mem_time1);
		total_mem_time += mem_time0 + mem_time1;


		fprintf(stderr, "Total processing time of the batch on GPU is %.3f seconds\n", realtime() - gpu_batch_time);
		total_gpu_time += (realtime() - gpu_batch_time);

		double print_time = realtime();
			...
			for (i = 0, j = 0, k = 0; i < total_reads; i++) {
				...
				for (y = 0;  y < n_ref_pos_fow_rev[i] && j < n_seeds_sum_fow_rev; j++, y++) {
					if (prev_seed_begin == seed_read_pos_fow_rev[j].x && prev_seed_end == (seed_read_pos_fow_rev[j].y)) {
						...
						fprintf(stdout,"\n");
						fprintf(stdout, "Read[%d, %d] -> %c%llu", seed_read_pos_fow_rev[j].x << 1 >> 1, (seed_read_pos_fow_rev[j].y << 1 >> 1), sign[((uint32_t)seed_read_pos_fow_rev[j].y) >> 31], seed_ref_pos_fow_rev[j]);
					}
...
//			uint32_t* packed_read_batch_rev_cpu = (uint32_t*)calloc((read_batch_size_8 >> 3), sizeof(uint32_t));
//			cudaMemcpy(packed_read_batn_seeds_sum_fow_rev_gpuch_rev_cpu, packed_read_batch_rev, (read_batch_size_8 >> 3)*sizeof(uint32_t), cudaMemcpyDeviceToHost);
//			uint32_t* packed_read_batch_fow_cpu = (uint32_t*)calloc((read_batch_size_8 >> 3), sizeof(uint32_t));
//			cudaMemcpy(packed_read_batch_fow_cpu, packed_read_batch_fow, (read_batch_size_8 >> 3)*sizeof(uint32_t), cudaMemcpyDeviceToHost);
//			for (i = 0; i < total_reads; i++) {
//				fprintf(stdout, "/*===================================SMEM seeds in read no. %d (Read[begin, End] -> SA[l, u] -> starting position(s) on the reference)===================================*/\n", reads_processed + i  + 1);
//				//fprintf(stdout, "\tOrig Read=");
//				//print_seq_ascii(read_sizes[i],  &(read_batch[read_offsets[i]]));
//				//fprintf(stdout,"\n");
//				//fprintf(stdout, "\tForward Read=");
//				//print_seq_packed(read_sizes[i]/*%8 ? read_sizes[i] + (8 - read_sizes[i]%8) : read_sizes[i]*/ , &(packed_read_batch_fow_cpu[(read_offsets[i]%8 ?  read_offsets[i] + (8 - read_offsets[i]%8) : read_offsets[i]) >> 3]), 8 - (read_sizes[i]&7));
//				//print_seq_packed(read_sizes[i]/*%8 ? read_sizes[i] + (8 - read_sizes[i]%8) : read_sizes[i]*/ , &(packed_read_batch_fow_cpu[read_offsets[i] >> 3]), read_offsets[i]&7, 0);
//				//fprintf(stdout,"\n");
//				//fprintf(stdout, "read_seq:");
//				//print_seq(read_sizes[i], &(read_batch[read_offsets[i]]));
//				//fprintf(stdout, "\n");
//				//fprintf(stdout, "--------------------------------------------------------------------------------\n");
//				int j;
//				int prev_seed_begin = MAX_READ_LENGTH;
//				bwtint_t prev_interval_size = 0;
//				for (j = 0; j < MAX_READ_LENGTH - min_seed_size; j++) {
//					int pass = 1;
//					int k;
//					//					for (k=0; k < num_seeds[i+1]; k++) {
//					//						int seed_read_begin_r_strand = read_sizes[i] - seed_read_end[((i+1)*(MAX_READ_LENGTH - min_seed_size)) + k];
//					//						int seed_read_end_r_strand = read_sizes[i] - seed_read_begin[((i+1)*(MAX_READ_LENGTH - min_seed_size)) + k];
//					//						if ((seed_read_begin[(i*(MAX_READ_LENGTH - min_seed_size)) + j] > seed_read_begin_r_strand && seed_read_end[(i*(MAX_READ_LENGTH - min_seed_size)) + j] < seed_read_end_r_strand)
//					//							|| (seed_read_begin[(i*(MAX_READ_LENGTH - min_seed_size)) + j] == seed_read_begin_r_strand && seed_read_end[(i*(MAX_READ_LENGTH - min_seed_size)) + j] < seed_read_end_r_strand)
//					//							|| (seed_read_begin[(i*(MAX_READ_LENGTH - min_seed_size)) + j] > seed_read_begin_r_strand && seed_read_end[(i*(MAX_READ_LENGTH - min_seed_size)) + j] == seed_read_end_r_strand))
//					//							pass = 0;
//					//					}
//					if (seed_intervals_fow[(i*(MAX_READ_LENGTH - min_seed_size)) + j].x > 0 &&  seed_intervals_fow[(i*(MAX_READ_LENGTH - min_seed_size)) + j].y > 0 &&  seed_intervals_fow[(i*(MAX_READ_LENGTH - min_seed_size)) + j].y >=  seed_intervals_fow[(i*(MAX_READ_LENGTH - min_seed_size)) + j].x ) {
//						fprintf(stdout,"\t\t\tRead[%d, %d] -> ", seed_read_pos_fow[(i*(MAX_READ_LENGTH - min_seed_size)) + j].x, seed_read_pos_fow[(i*(MAX_READ_LENGTH - min_seed_size)) + j].y);
//						//print_seq(seed_read_end[(i*(MAX_READ_LENGTH - min_seed_size)) + j] - seed_read_begin[(i*(MAX_READ_LENGTH - min_seed_size)) + j], &(read_batch[read_offsets[i] + seed_read_begin[(i*(MAX_READ_LENGTH - min_seed_size)) + j]]));
//						//print_seq(read_sizes[i], &(read_batch[read_offsets[i]]));
//
//						fprintf(stdout,"\t-> SA[%llu, %llu]\t->", seed_intervals_fow[(i*(MAX_READ_LENGTH - min_seed_size)) + j].x, seed_intervals_fow[(i*(MAX_READ_LENGTH - min_seed_size)) + j].y);
//						//						int m;
//						//						for(m = 0; m < seed_intervals_u[(i*(MAX_READ_LENGTH - min_seed_size)) + j] - seed_intervals_l[(i*(MAX_READ_LENGTH - min_seed_size)) + j] + 1; m++, q++){
//						//							fprintf(stdout," %d,", seed_ref_begin[(i*MAX_SEEDS_PER_READ) + q]);
//						//						}
//						fprintf(stdout,"\n");
//					}
//					//prev_seed_begin = seed_read_begin[(i*(MAX_READ_LENGTH - min_seed_size)) + j];
//					//prev_interval_size = (seed_intervals_u[(i*(MAX_READ_LENGTH - min_seed_size)) + j] - seed_intervals_l[(i*(MAX_READ_LENGTH - min_seed_size)) + j] + 1);
//				}
//				fprintf(stdout, "\t-----------------------------------------------------------\n");
//				fprintf(stdout, "\tRerverse Read=");
//				fprintf(stdout,"\n");
//				//print_seq_packed(read_sizes[i]/*%8 ? read_sizes[i] + (8 - read_sizes[i]%8) : read_sizes[i]*/ , &(packed_read_batch_rev_cpu[read_offsets[i] >> 3]), read_offsets[i]&7);
//				fprintf(stdout,"\n");
//				//				int prev_seed_begin = MAX_READ_LENGTH;
//				//				bwtint_t prev_interval_size = 0;
//				for (j = 0; j < MAX_READ_LENGTH - min_seed_size; j++) {
//					int pass = 1;
//					int k;
//					int seed_read_begin_r_strand = read_sizes[i] - seed_read_pos_rev[(i*(MAX_READ_LENGTH - min_seed_size)) + j].y;
//					int seed_read_end_r_strand = read_sizes[i] - seed_read_pos_rev[(i*(MAX_READ_LENGTH - min_seed_size)) + j].x;
//					//			    	for (k=0; k < num_seeds[i-1]; k++) {
//					//			    		if (( seed_read_begin_r_strand > seed_read_begin[((i-1)*(MAX_READ_LENGTH - min_seed_size)) + k] && seed_read_end_r_strand < seed_read_end[((i-1)*(MAX_READ_LENGTH - min_seed_size)) + k])
//					//			    				|| ( seed_read_begin_r_strand == seed_read_begin[((i-1)*(MAX_READ_LENGTH - min_seed_size)) + k] && seed_read_end_r_strand < seed_read_end[((i-1)*(MAX_READ_LENGTH - min_seed_size)) + k])
//					//			    				|| ( seed_read_begin_r_strand > seed_read_begin[((i-1)*(MAX_READ_LENGTH - min_seed_size)) + k] && seed_read_end_r_strand == seed_read_end[((i-1)*(MAX_READ_LENGTH - min_seed_size)) + k]))
//					//			    			pass = 0;
//					//			    	}
//					if (seed_intervals_rev[(i*(MAX_READ_LENGTH - min_seed_size)) + j].x > 0 &&  seed_intervals_rev[(i*(MAX_READ_LENGTH - min_seed_size)) + j].y > 0 &&  seed_intervals_rev[(i*(MAX_READ_LENGTH - min_seed_size)) + j].y >=  seed_intervals_rev[(i*(MAX_READ_LENGTH - min_seed_size)) + j].x ) {
//						fprintf(stdout,"\t\t\tRead[%d, %d] -> ", seed_read_pos_rev[(i*(MAX_READ_LENGTH - min_seed_size)) + j].x, seed_read_pos_rev[(i*(MAX_READ_LENGTH - min_seed_size)) + j].y);
//						//fprintf(stdout,"\t\t\tRead[%d, %d] -> ", seed_read_begin_r_strand, seed_read_end_r_strand);
//						//print_seq(seed_read_end[(i*(MAX_READ_LENGTH - min_seed_size)) + j] - seed_read_begin[(i*(MAX_READ_LENGTH - min_seed_size)) + j], &(read_batch[read_offsets[i] + seed_read_begin[(i*(MAX_READ_LENGTH - min_seed_size)) + j]]));
//						//print_seq(read_sizes[i], &(read_batch[read_offsets[i]]));
//						fprintf(stdout,"\t-> SA[%llu, %llu]\t->", seed_intervals_rev[(i*(MAX_READ_LENGTH - min_seed_size)) + j].x, seed_intervals_rev[(i*(MAX_READ_LENGTH - min_seed_size)) + j].y);
//						//						int m;
//						//						for(m = 0; m < seed_intervals_u[(i*(MAX_READ_LENGTH - min_seed_size)) + j] - seed_intervals_l[(i*(MAX_READ_LENGTH - min_seed_size)) + j] + 1; m++, q++){
//						//							fprintf(stdout," %d,", seed_ref_begin[(i*MAX_SEEDS_PER_READ) + q]);
//						//						}
...
//					//				if (1/*prev_seed_begin > seed_read_begin[(i*(MAX_READ_LENGTH - min_seed_size)) + j] /*&& prev_interval_size != (seed_intervals_u[(i*(MAX_READ_LENGTH - min_seed_size)) + j] - seed_intervals_l[(i*(MAX_READ_LENGTH - min_seed_size)) + j] + 1)*/) {
//					//					fprintf(stdout,"\t\t\tRead[%d, %d] -> ", seed_read_begin[(i*(MAX_READ_LENGTH - min_seed_size)) + j], seed_read_end[(i*(MAX_READ_LENGTH - min_seed_size)) + j]);
//					//					print_seq(seed_read_end[(i*(MAX_READ_LENGTH - min_seed_size)) + j] - seed_read_begin[(i*(MAX_READ_LENGTH - min_seed_size)) + j], &(read_batch[read_offsets[i] + seed_read_begin[(i*(MAX_READ_LENGTH - min_seed_size)) + j]]));
//					//					//print_seq(read_sizes[i], &(read_batch[read_offsets[i]]));
//					//					fprintf(stdout,"\t-> SA[%d, %d]\n", seed_intervals_l[(i*(MAX_READ_LENGTH - min_seed_size)) + j], seed_intervals_u[(i*(MAX_READ_LENGTH - min_seed_size)) + j]);
//					//				}
//					//				prev_seed_begin = seed_read_begin[(i*(MAX_READ_LENGTH - min_seed_size)) + j];
//					//				prev_interval_size = (seed_intervals_u[(i*(MAX_READ_LENGTH - min_seed_size)) + j] - seed_intervals_l[(i*(MAX_READ_LENGTH - min_seed_size)) + j] + 1);
//					//			}
...
//		free(read_num); free(read_idx);
		total_print_time += (realtime() - print_time);
